...
def get_history_points(db: Session, job_id: str) -> List[HistoryPoint]:
    ...
    return [HistoryPoint(iteration=p.iteration, value=p.value, qubo_energy=p.qubo_energy, entropy=p.entropy, is_feasible=p.is_feasible, qubit_probs=p.qubit_probs) for p in points]
